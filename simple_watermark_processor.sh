...
# Simple PDF Watermark Processor for mySafePlay
...
echo "- Footers: Document path starting with relative path on left, page number on right"
...
while IFS='|' read -r pdf_path size; do
    ...
    # Get relative path (remove /home/ubuntu prefix)
...
MDEOF
    ...
    # Run pandoc to create watermarked PDF
    if pandoc "$temp_md" \
        --template="$TEMPLATE" \
        -V created="$formatted_date" \
        -V relativepath="$relative_path" \
        --pdf-engine=pdflatex \
...
echo "========================================================"
echo "All PDF files in /home/ubuntu/ directory have been processed!"
